...
from depthcharge.components import ModelMixin, PeptideDecoder, RecyclingSpectrumEncoder
    ...
    **kwargs : Dict
    ...
    def __init__(
        ...
        **kwargs: Dict,
        ...
        # Build the model.
        self.encoder = RecyclingSpectrumEncoder(
            dim_model=dim_model,
            n_head=n_head,
    ...
    def beam_search_decode(
        self, spectra: torch.Tensor, precursors: torch.Tensor, embeddings: Optional[torch.Tensor], loops: int
    ) -> List[List[Tuple[float, np.ndarray, str]]]:
        """
        ...
        precursors : torch.Tensor of size (n_spectra, 3)
            The measured precursor mass (axis 0), precursor charge (axis 1), and
            precursor m/z (axis 2) of each MS/MS spectrum.
        embeddings : torch.Tensor of shape (n_spectra, dim_embedding), optional
        ...
        intermediate = list(self._get_top_peptide(pred_cache))
        if loops <= 0:
            return intermediate
        intermediate = [top_peptides[0][2] for top_peptides in intermediate]
        intermediate = [("", peptide) for peptide in intermediate]
    ...
    def _forward_step(
        ...
        """
        return self.decoder(sequences, precursors, *self.encoder(spectra, None))
